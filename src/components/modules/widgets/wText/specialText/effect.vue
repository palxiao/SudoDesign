 ...
 * @LastEditTime: 2022-02-20 21:48:10
...
<template>
  <div class="effect__text" :style="{ '--stroke_1': stroke + 'px', '--stroke_2': stroke * ratio + 'px' }">
    ...
    <div :style="{ fontFamily: params?.fontClass?.value }" class="editable-text">{{ params.text }}</div>
  </div>
  <!-- <div class="t" :style="{ '--stroke_1': stroke + 'px', '--stroke_2': stroke * ratio + 'px' }">
    <div
      :style="{
...
export default defineComponent({
  ...
  setup(props) {
    const ratio = ref(5 / 2) // 强度依据的图层宽度比值
...
.t2 {
  // font-weight: normal;
  color: #d42a2a;
  -webkit-text-stroke: var(--stroke_1) #d42a2a;
}
.t3 {
  // font-weight: normal;
  color: #f1d145;
  -webkit-text-stroke: var(--stroke_2) #f1d145;
