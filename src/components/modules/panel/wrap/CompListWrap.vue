...
export default defineComponent({
  ...
  methods: {
    ...
    async dragStart(e: any, { id }: any) {
      const res = await api.home.getTempDetail({ id, type: 1 })
      if (Array.isArray(JSON.parse(res.data))) {
        this.$store.commit('selectItem', { data: JSON.parse(res.data), type: 'group' })
      } else {
        this.$store.commit('selectItem', { data: JSON.parse(res.data), type: 'text' })
      }
    },
  },
})
